...
namespace Pyton
{
    ...
    class ConsoleWrite
    {
        ...
        static void Main(string[] args)
        {
            ...
            Ap.PrintEv += (new Obj.ConsolDelegate(PrintAp));
            Ap.CordProv2+=(new Obj.ConsolDelegate2(CordProvAp));
            Ap.Updete1+=(new Obj.ConsolDelegate3(Updete));

         


            bool worc_map_gener = true;
            int Height=20;
            int Width=20;
            do
            {
                try
                {
                    ...
                    if (Width < 5)
                        throw new Exception("Error valy Width, please enter to valy big is 5");

                    Console.Write("Please enter Height:");
                     Height = Int32.Parse(Console.ReadLine());
                    if (Height < 5)
                        throw new Exception("Error valy Height, please enter to valy big is 5");
                   
                  
                    worc_map_gener = false;
                }
                catch (Exception e)
                {
                    Console.Clear();
            ...
            } while (worc_map_gener);
            ObjMap = new Map(Width, Height);
            ObjMap.Print += (new Map.ConsolDelegate(PrintMap));
            ObjMap.print();

            Pl.Print+=(new Snake.ConsolDelegate(PrintSnake));
            Pl.noPrint += ( new Snake.ConsolDelegate2(PrintSnakeEnd));
            Pl.noPrint += (new Snake.ConsolDelegate2(CordProvSnake));
